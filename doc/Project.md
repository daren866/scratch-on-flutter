...
| `md5ext` | string | MD5+扩展名，用于查找ZIP中的文件 | `"7da4181ee167de7b.svg"` |
...
这些属性定义了角色在舞台上的显示状态和位置。

##### 1.1.11.1 位置属性

| 属性 | 类型 | 取值范围 | 默认值 | 说明 |
|------|------|---------|--------|------|
| `x` | number | -240 ~ 240 | `0` | X坐标（向右为正） |
| `y` | number | -180 ~ 180 | `0` | Y坐标（向上为正） |
...
| 属性 | 类型 | 取值范围 | 默认值 | 说明 |
|------|------|---------|--------|------|
| `direction` | number | -179 ~ 180 | `90` | 朝向角度 |
...
| 属性 | 类型 | 可选值 | 默认值 | 说明 |
|------|------|--------|--------|------|
| `rotationStyle` | string | 见下文 | `"all around"` | 旋转方式 |

**旋转样式取值**：

| 值 | 中文名称 | 效果说明 | 适用场景 |
|----|---------|---------|---------|
| `"all around"` | 任意旋转 | 角色可以旋转到任意方向 | 自由旋转的角色 |
| `"left-right"` | 左右翻转 | 只在左右方向翻转，不旋转 | 走路/跑步角色 |
| `"don't rotate"` | 不旋转 | 始终保持初始方向 | HUD/图标类元素 |

**示例**：
```json
"rotationStyle": "left-right"  // 角色只能左右翻转
```
...
| `size` | number | 5 ~ 2300 | `100` | 大小百分比 |
| `draggable` | boolean | `true`/`false` | `false` | 运行时是否可拖拽 |
...
**含义**：当前应用的图形特效值（保存在内存中，不直接存储在 JSON）

| 特效名称 | 英文名 | 取值范围 | 默认值 | 说明 |
|---------|--------|---------|--------|------|
| `color` | 颜色 | 0 ~ 100 | `0` | 色调偏移 |
| `fisheye` | 鱼眼 | 0 ~ 100 | `0` | 鱼眼镜头效果 |
| `whirl` | 漩涡 | -180 ~ 180 | `0` | 旋转扭曲角度 |
| `pixelate` | 像素化 | 0 ~ 100 | `0` | 像素化程度 |
| `mosaic` | 马赛克 | 0 ~ 100 | `0` | 马赛克程度 |
| `brightness` | 亮度 | -100 ~ 100 | `0` | 亮度调整 |
| `ghost` | 虚像 | 0 ~ 100 | `0` | 透明度（0=不透明，100=完全透明） |

**特效说明**：
- 所有特效值都是相对于默认状态的偏移量
- 可以通过积木 `looks_changeeffectby` 和 `looks_seteffectto` 修改
- 可以通过 `looks_cleargraphiceffects` 重置所有特效

**在 JSON 中的存储**：
```json
// 注意：特效值通常不直接存储在 project.json 中
// 它们是运行时的临时状态
// 仅在需要保存特效状态时可能会包含
```
...
| 属性 | 类型 | 取值范围 | 默认值 | 说明 |
|------|------|---------|--------|------|
| `layerOrder` | number | 整数 | `0` | 图层顺序，0表示最底层 |
| `volume` | number | 0 ~ 100 | `100` | 音量百分比 |
| `tempo` | number | 20 ~ 300 | `60` | 速度（BPM，每分钟节拍数） |
| `videoTransparency` | number | 0 ~ 100 | `50` | 视频透明度 |
| `videoState` | string | 见下文 | `"on"` | 视频状态 |
| `textToSpeechLanguage` | string/null | 语言代码 | `null` | 语音语言 |

**tempo 属性说明**：
- 控制 "演奏鼓声" 和 "演奏音符" 积木的速度
- 单位：BPM（每分钟节拍数）
- 有效范围：20 ~ 300 BPM
...
| 属性 | 类型 | 取值范围 | 默认值 | 说明 |
|------|------|---------|--------|------|
| `layerOrder` | number | 整数 ≥ 1 | - | 图层顺序，越大越在上面 |
| `volume` | number | 0 ~ 100 | `100` | 音量百分比 |
| `visible` | boolean | `true`/`false` | `true` | 是否显示 |
| `x` | number | -240 ~ 240 | `0` | X坐标 |
| `y` | number | -180 ~ 180 | `0` | Y坐标 |
| `size` | number | 5 ~ 2300 | `100` | 大小百分比 |
| `direction` | number | -179 ~ 180 | `90` | 方向角度 |
| `draggable` | boolean | `true`/`false` | `false` | 是否可拖拽 |
| `rotationStyle` | string | 见旋转样式表 | `"all around"` | 旋转方式 |

#### 1.1.16 克隆相关属性

| 属性 | 类型 | 说明 |
|------|------|------|
| `isOriginal` | boolean | 是否是原始角色（非克隆体） |
| `isStage` | boolean | 是否是舞台 |

**说明**：
- `isOriginal: true` = 在编辑器中创建的角色，或舞台
- `isOriginal: false` = 通过 "克隆" 积木创建的克隆体
- 克隆体不存储在 project.json 中，仅在运行时存在

**示例**：
```json
"isOriginal": true,  // 这是原始角色
"isStage": false      // 这是角色，不是舞台
```

#### 1.1.17 图层顺序（layerOrder）

**含义**：控制角色的前后遮挡关系

**规则**：
- 数值越大，角色显示在越上层
- 舞台的 layerOrder 固定为 0
- 角色的 layerOrder 从 1 开始递增
- 可以通过 "移到最上层/最下层" 和 "上移/下移" 积木改变

**示例**：
```json
// 在 targets 数组中的顺序也反映了 layerOrder
"targets": [
  { "name": "Stage", "layerOrder": 0, ... },    // 最底层
  { "name": "背景2", "layerOrder": 1, ... },
  { "name": "角色A", "layerOrder": 2, ... },    // 在背景2上层
  { "name": "角色B", "layerOrder": 3, ... }     // 最上层
]
```
  ...
  "costumes": [
    {
      ...
      "bitmapResolution": 1,
      "dataFormat": "svg",
      "assetId": "739b3e2b4c13f2f58f8b8a56c1d45f2",
      "md5ext": "739b3e2b4c13f2f58f8b8a56c1d45f2.svg",
      "rotationCenterX": 240,
      "rotationCenterY": 180
    }
  ],
  ...
  "costumes": [
    {
      ...
      "md5ext": "7da4181ee167de7b3f5d1a91880277ff.png",
      "rotationCenterX": 47,
      "rotationCenterY": 55
    }
  ],
...
for (const monitor of project.monitors) {
  console.log(`监视器: ${monitor.opcode}`);
}
```
...
### 5.5 坐标系统
- Scratch舞台宽度：480像素（X: -240 ~ 240）
- Scratch舞台高度：360像素（Y: -180 ~ 180）
- 中心点：(0, 0)
- 右边界X：240
- 左边界X：-240
- 上边界Y：180
...
### 5.6 图层顺序
- 舞台永远是 layerOrder 0
- 角色的 layerOrder 从 1 开始
- 数值越大显示在上层
- 克隆体继承原始角色的 layerOrder

### 5.7 特效状态
- 特效值是运行时状态，不直接保存在 JSON
- 停止运行时特效重置为默认值
- 使用 `清除图形特效` 积木可手动重置
